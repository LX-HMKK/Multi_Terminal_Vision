...
> 搭载在「动量轮自平衡自行车」上的多端视觉避障与远程控制系统——Jetson Nano 车端 + YOLO 运算端 + Qt 上位机，覆盖采集、传输、检测、跟踪、避障到控制的完整闭环。
...
上位机为**响应式两列卡片布局**，可切换**浅色/暗色主题**（`CAR_THEME=dark` 可强制暗色）。
...
│   └── src/           # Qt 源码（main / widget / tcp / udp / message_store）
├── docs/              # 架构 / 协议 / 已知问题
└── tools/             # 冒烟测试等
```
...
```
`--source`：`udp`(nano，默认) / `webcam` / `video`（配合 `--input <file>`）/ `synthetic`(合成帧)。
